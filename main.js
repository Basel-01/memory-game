...
document.querySelector(".best-score .name").innerHTML = localStorage.getItem("name");
document.querySelector(".best-score .best").innerHTML = localStorage.getItem("wrong");
...
function startGame(name) {
  ...
  blocks.forEach((block,index) => {
    ...
    block.addEventListener("click", _ => {
      ...
      let flipedBlocks = blocks.filter(block => block.classList.contains("fliped"));
      if(flipedBlocks.length === 2) {
        ...
        if(flipedBlocks[0].dataset.brand === flipedBlocks[1].dataset.brand) {
          ...
          setTimeout(_ => {
            ...
            flipedBlocks.forEach(flipedBlock => flipedBlock.classList.remove("fliped"));
          }, 1000);
        }
      }
      let allMathced = blocks.filter(block => block.classList.contains("done"));
      if(allMathced.length === blocks.length) {
        document.getElementById('win').play();
        if(wrongTries.innerHTML <= window.localStorage.getItem("wrong") || window.localStorage.getItem("wrong") === null) {
          window.localStorage.setItem("wrong", wrongTries.innerHTML);
          window.localStorage.setItem("name", name);
          document.querySelector(".best-score .name").innerHTML = wrongTries.innerHTML;
